...
int main(int argc, char *argv[])
{
    ...
    while (getline(file_2, codeline))
    {
        ...
        end_brace = "";
        for (int io = codeline.length() - 1; i >= 0; io--)
        {
            if (codeline[io] != ' ' && codeline[io] != '{')
            {
                end_brace = codeline.substr(io + 1, codeline.length());
                codeline = codeline.substr(0, io + 1);
                break;
            }
        }
